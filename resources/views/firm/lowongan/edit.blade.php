<x-layout
    ...
    title="Edit lowongan"
    subtitle="Perbarui informasi lowongan magang."
    :menu="\App\Support\SidebarMenu::firm('lowongan')"
    :user-meta="'Advokat Pendamping · '.$firm->nama"
>
    <x-card class="p-6 max-w-3xl">
        <form method="POST" action="{{ route('firm.lowongan.update', $lowongan) }}" class="flex flex-col gap-5">
            @csrf
            @method('PATCH')
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-2" x-data="{ bidang: @json(old('bidang', $lowongan->bidang ?? [])) }">
